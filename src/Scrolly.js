...
export default {
  ...
  methods: {
    ...
    onMouseDown({ target: bar, pageX: initialPageX, pageY: initialPageY }) {
      ...
      const self = this;
      const { barXt, barXb, barY } = this;
      const { container, viewport } = this;
      const { addEventListener, removeEventListener } = window;
      const isAxisXt = className.match('axis-x_top');
      const isAxisXb = className.match('axis-x_bottom');
    ...
    refreshScrollLayout(dx = 0, dy = 0) {
      ...
      const { viewport, barXt, barXb, barY } = this;
      ...
      if (barXt) {
        ...
        const barWidth = barXt.offsetWidth;
        ...
        let previousScrollLayout = barXt.scrollLayout || {};
        let {
          onEdge: wasOnEdge,
      ...
      if (barXb) {
        ...
        // Get computed bar width
        const barWidth = barXb.offsetWidth;
        ...
        // Get previous scroll layout to determine
        // if we can unlock parent scrolling
        let previousScrollLayout = barXb.scrollLayout || {};
        let {
          onEdge: wasOnEdge,
          canUnlockParentScroll: couldUnlockParentScroll,
          canScrollParent: couldScrollParent,
